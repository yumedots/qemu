...
/* MAP_FIXED swaps host pages underneath an already-mapped ramblock.  On
 * Linux/KVM the mmu_notifier invalidates the stale EPT entries; macOS/HVF
 * has no equivalent notification, so the guest keeps accessing the old
 * anonymous pages and Venus ring/blob contents silently diverge.  Use the
 * blob-subregion fallback there, which adds a MemoryRegion and forces the
 * accelerator to re-map guest memory.
 */
#define VIRGL_HAS_MAP_FIXED \
    (VIRGL_CHECK_VERSION(1, 3, 0) && !IS_ENABLED(CONFIG_WIN32) && !defined(__APPLE__))
                                   ...
                                   uint64_t offset)
{
    ...
    /*
     * Under HVF the mapped blob is added as guest RAM and backed with
     * hv_vm_map(), which requires host-page alignment (16 KiB on Apple
     * Silicon) of the host pointer, the guest physical offset and the
     * size.  Blob offsets and sizes are guest-page (4 KiB) granular and
     * glMapBufferRange() host pointers are arbitrary, so an unaligned
     * blob cannot be backed here: reject the map and unpin the virgl
     * mapping.  The guest gets VIRTIO_GPU_RESP_ERR_UNSPEC and falls back
     * to transfer-based access to the resource instead of crashing the
     * accelerator's memory listener.
     */
    if (hvf_enabled() &&
        (((uintptr_t)data | size | offset) &
         (qemu_real_host_page_size() - 1))) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "%s: blob %" PRIu32 " is not host-page aligned "
                      "(ptr=%p size=%" PRIu64 " offset=%" PRIu64 "), "
                      "rejecting map\n",
                      __func__, res->base.resource_id, data, size, offset);
...
#if VIRGL_VERSION_MAJOR >= 1
    /* Fences on a non-zero context must retire against that context's
     * timeline, not the global ctx0 timeline: in render-server mode the
     * global vrend queue never sees the context's work, so a ctx0 fence
     * for a Venus submission would never retire and the guest would stall
     * on vkQueueSubmit fences.  ring_idx is 0 for the main ring (retired
     * on the cpu timeline) and comes from the header for TLS rings.
     */
    if (cmd->cmd_hdr.ctx_id) {
