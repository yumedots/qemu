...
static void hvf_set_phys_mem(MemoryRegionSection *section, bool add)
{
    ...
    if (memory_region_is_ram(area) &&
        ...
        /*
         * Not host-page aligned, so we can not map it as RAM.  The section
         * was never mapped via hv_vm_map() (only aligned RAM ever gets
         * mapped), so do NOT fall through to hv_vm_unmap(): hv_vm_unmap()
         * of a never-mapped range fails with HV_BAD_ARGUMENT and the
         * assert would abort the whole VM.  Warn once and skip.
         */
        static bool warned;
        if (!warned) {
            qemu_log_mask(LOG_UNIMP,
                          "hvf: skipping unaligned RAM section gpa 0x%"
                          HWADDR_PRIx " size 0x%" PRIx64 "\n", gpa, size);
            warned = true;
        }
        return;
    }
